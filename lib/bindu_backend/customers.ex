defmodule BinduBackend.Customers do
  @moduledoc """
  ...
  def subscribe_customers(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(BinduBackend.PubSub, "user:#{key}:customers")
  end

  defp broadcast_customer(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(BinduBackend.PubSub, "user:#{key}:customers", message)
  end
